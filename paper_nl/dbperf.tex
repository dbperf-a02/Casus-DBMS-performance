...
Om postgre te installeren, voert u als root user (su -) dit commando uit :
  ...
  \framebox[1.1\width]{yum install https://download.postgresql.org/pub/repos/yum/10/redhat/rhel-7-x86_64/pgdg-centos10-10-2.noarch.rpm} \par
...
Daar voert u 
  \framebox[1.1\width]{vi pg_hba_conf}\par
...
Daar voert u het script uit met 
 \framebox[1.1\width]{ psql -U dbo -f create_tables.sql (dit moet in de gewone vagrant command prompt(links onderaan))}\par
